...
class GUI(Tk):
    ...
    def __init__(self, dim=200, model="forest_clf.sav", *args, **kwargs):
        ...
        self.canvas = Canvas(self, height=dim_dict[dim], width=dim_dict[dim], bg="white")
        self.img_canvas = Canvas(self, height=24.4999999999, width=24.4999999999)
        self.predict_button = Button(self, text="Predict", command=self.predict_digit, height=2, width=10)
        self.clear_button = Button(self, text="Clear", command=self.clear_screen, height=2, width=10)
        self.recognizer = Recognizer()
        self.prediction_label = Label(self, text="Draw and click predict", width=20, height=1)
        self.scale_label_frame = LabelFrame(self, text="Brush size", bd=5, font=('arial', 7, 'bold'), relief=RIDGE)
